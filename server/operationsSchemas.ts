...
export const updatePropertySchema = createPropertySchema.partial().omit({ organizationId: true, projectId: true, propertyReference: true }).extend({ acquisitionStatus: acquisitionStatusSchema.optional(), legalStatus: z.enum(['unknown','clear','under_review','blocked']).optional(), documentationStatus: z.string().trim().max(80).optional(), paymentStatus: z.string().trim().max(80).optional(), readinessPercent: z.number().int().min(0).max(100).optional() });
